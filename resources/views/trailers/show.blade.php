        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <!-- Photo (for public view; one per trailer) -->
                    <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
                        <h3 class="text-lg font-semibold mb-4">Photo (shown on public trailer listing)</h3>
                        @can('update', $trailer)
                        <form action="{{ url("/trailers/{$trailer->id}/photos") }}" method="POST" enctype="multipart/form-data" class="mb-6">
                            @csrf
                            <div class="flex flex-wrap items-end gap-4">
                                <div class="min-w-0 flex-1">
                                    <label for="photo" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Upload or URL</label>
                                    ...
                                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">JPEG, PNG, GIF or WebP. Max 5 MB. Or use a URL below.</p>
                                </div>
                                <div class="min-w-0 flex-1">
                                ...
                                <button type="submit" class="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-md text-sm font-medium">{{ $trailer->photos->count() > 0 ? 'Replace photo' : 'Add photo' }}</button>
                            </div>
                        </form>
                        @endcan
                        @php $p = $trailer->photos->first(); @endphp
                        @if($p)
                        <div class="relative group rounded-lg overflow-hidden border border-gray-200 dark:border-gray-600 max-w-md">
                            @if($p->image_url)
                            ...
                            <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition flex items-center justify-center">
                                <form action="{{ url("/trailers/{$trailer->id}/photos/{$p->id}") }}" method="POST" class="inline" onsubmit="return confirm('Remove this photo?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="bg-red-600 text-white px-4 py-2 rounded text-sm font-medium hover:bg-red-700">Remove</button>
                                </form>
                            </div>
                            @endcan
                        </div>
                        @else
                        <p class="text-gray-500 dark:text-gray-400 text-sm">No photo yet. Add one to display it on the public trailer listing.</p>
                        @endif
                    </div>
