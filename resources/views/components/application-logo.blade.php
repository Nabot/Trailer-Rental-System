...
@if($logoPath && file_exists(public_path($logoPath)))
    <img src="{{ asset($logoPath) }}" alt="{{ $companyName }}" {{ $attributes->merge(['class' => 'h-28 w-auto']) }} />
@elseif(file_exists(public_path('images/ironaxle-logo.png')))
    <img src="{{ asset('images/ironaxle-logo.png') }}" alt="{{ $companyName }}" {{ $attributes->merge(['class' => 'h-28 w-auto']) }} />
@elseif(file_exists(public_path('images/ironaxle-logo.svg')))
    <img src="{{ asset('images/ironaxle-logo.svg') }}" alt="{{ $companyName }}" {{ $attributes->merge(['class' => 'h-28 w-auto']) }} />
@else
